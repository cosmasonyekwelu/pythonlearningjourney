...
class TradeAPI(Resource):
    ...
    def get(self, trade_id=None):
        try:
            ...
            if trade_id:
                # Get single trade
                trade = Trade.query.filter_by(id=trade_id, user_id=current_user_id).first()
                if trade:
        ...
        try:
            ...
            for field in allowed_fields:
                if field in data:
                    ...
                    setattr(trade, field, data[field].upper() if field in ['symbol', 'trade_type'] else data[field])
    ...
    def get(self):
        try:
            ...
            for symbol, data in portfolio.items():
                if data['quantity'] > 0:
                    data['average_price'] = data['total_invested'] / data['quantity']
                    # Mock current price (in real app, fetch from market data API)
                    current_price = data['average_price'] * (1 + (hash(symbol) % 20 - 10) / 100)
                    data['current_price'] = round(current_price, 2)
                    data['current_value'] = data['quantity'] * data['current_price']
                    data['pnl'] = data['current_value'] - data['total_invested']
                    data['pnl_percent'] = (data['pnl'] / data['total_invested']) * 100 if data['total_invested'] > 0 else 0
...
class MarketDataAPI(Resource):
    def get(self):
        try:
            symbols = request.args.get('symbols', 'AAPL,GOOGL,MSFT,TSLA,AMZN').split(',')
            ...
            for symbol in symbols:
                ...
                if symbol:
                    ...
                    change = (hash(symbol + str(datetime.now().hour)) % 40 - 20) / 2
...
def create_sample_data():
    ...
    with app.app_context():
        ...
        if not user:
            user = User(username='demo', email='demo@example.com', password='password')
            db.session.add(user)
            db.session.commit()
        ...
        sample_trades = [
            ...
            {'symbol': 'GOOGL', 'quantity': 5, 'price': 2750.00, 'trade_type': 'BUY'},
